...
import { PoolState, usePool } from 'app/hooks/v3/usePools';
import {
  WETH9,
  Currency,
  CurrencyAmount,
...
import { Rounding } from '../../../v3-sdk/enums/rounding';
...
import { BIG_INT_ZERO, CHAIN_ID } from '../../../constants';
...
export function useActivePreset(): RootState['mintV3']['preset'] {
  const preset = useAppSelector((state: RootState) => state.mintV3.preset);
  return useMemo(() => preset, [preset]);
}

export function useAddLiquidityTxHash(): RootState['mintV3']['txHash'] {
  const txHash = useAppSelector((state: RootState) => state.mintV3.txHash);
  return useMemo(() => txHash, [txHash]);
}

export function useShowNewestPosition(): RootState['mintV3']['showNewestPosition'] {
  const newestPosition = useAppSelector(
    (state: RootState) => state.mintV3.showNewestPosition,
  );
  return useMemo(() => newestPosition, [newestPosition]);
}

export function useInitialUSDPrices(): RootState['mintV3']['initialUSDPrices'] {
  const initialUSDPrices = useAppSelector(
    (state: RootState) => state.mintV3.initialUSDPrices,
  );
  return useMemo(() => initialUSDPrices, [initialUSDPrices]);
}

export function useInitialTokenPrice(): RootState['mintV3']['initialTokenPrice'] {
  const initialTokenPrice = useAppSelector(
    (state: RootState) => state.mintV3.initialTokenPrice,
  );
  return useMemo(() => initialTokenPrice, [initialTokenPrice]);
}

export function useCurrentStep(): RootState['mintV3']['currentStep'] {
  const currentStep = useAppSelector(
    (state: RootState) => state.mintV3.currentStep,
  );
